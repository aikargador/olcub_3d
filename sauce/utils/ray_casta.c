...
//one part should become divisible by blk_sz so thats how u know u reached
// so for x part, the y axis should become divisible by blk_sz!
...
// 		del->y = sin(angel) * 5;
// 		// for (float g = 1; g; g+= .1) {
// 		// 	int pp = ((int)((xy->y + (del->y * g)) / BLK)) * 8;
// 		// 	pp += (int)((xy->x + (del->x * g)) / BLK);
...
// 		// } for (float g = 1; g; g+= .1) {
// 		// 	int pp = ((int)((table->y + (table->del_y * g)) / BLK)) * 8;
// 		// 	pp += (int)((table->x + (table->del_x * g)) / BLK);
// 		// 	if (table->map->map[pp] == 1) {
// 		// 		break ;
// 		// 	}
...
void	draw_ray(t_able *table, double rx, double ry, int clr)
{
	int	dx = rx - table->x;
	int	dy = ry - table->y;
...
void	draw_rayx(t_able *table, t_cords *xy, double rx, double ry, int clr)
{
	int	dx = rx - xy->x;
	int	dy = ry - xy->y;
...
// need the intersection! so div by BLK to find if u met an intersection!

// void	le_ray_caste_angel(t_able *table, t_cords *xy, t_cords *del, double ang) {//attempt no. 96335663263696969
...
// void	le_ray_caste_angel(t_able *table, t_cords *xy, t_cords *del, double ang) { //complete but gae!
...
// 			rx += del_rx;
// 			ry += del_ry;
// 			int chk = (((int)(ry + del_rx) / BLK) * 8) + ((int)(rx + del_rx) / BLK);
...
// 		any = sin(ang) * 5;
// 		for (float g = 1; g; g+= .1) {
// 			int pp = ((int)((table->y + (any * g)) / BLK)) * 8;
// 			pp += (int)((table->x + (anx * g)) / BLK);
// 			if (table->map->map[pp] == 1)
// 				break ;
// 			if (g == 1) {
// 				printf("bx*g => %f\n", (table->x + anx * g));
...
// void	le_ray_caste(t_able *table, t_cords *xy, t_cords *del) {
// 	double	angel;//angle bruh
// 	double	anx;
// 	double	any;
